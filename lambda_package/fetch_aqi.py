...
# ✅ Use Lambda environment variables instead of .env
...
def insert_into_db(records):
    ...
    rows = [
        (
            ...
            safe_float(rec.get("max_value")),
            safe_float(rec.get("avg_value")),
            parse_datetime(rec.get("last_update")),
        )
        for rec in records
    ]
    ...
    cur.executemany(
        ...
        (city, station, pollutant_id, pollutant_min, pollutant_max, pollutant_avg, last_update)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT DO NOTHING;
        """,
        rows,
...
def lambda_handler(event, context):
    ...
    for city in CITIES:
        try:
            ...
            inserted = insert_into_db(records)
            results[city] = f"Inserted {inserted} records"
